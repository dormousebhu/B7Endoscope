...
void CamerasDisplayWidget::loadSettings(QSettings &settings)
{
    for(int i = 0; i < 7; i++)
    {
        m_zoomFactorIndex[i] = settings.value(QString("ZoomFactor/CH%1").arg(i), 5).toInt();
    }
...
void CamerasDisplayWidget::writeSettings(QSettings &settings)
{
    for(int i = 0; i < 7; i++)
    {
        settings.setValue(QString("ZoomFactor/CH%1").arg(i), m_zoomFactorIndex[i]);
    }
}
